...
export function getAIConfig(env: Record<string, string | undefined> = process.env) {
...
export function validateAIEnvironment(env: Record<string, string | undefined> = process.env) { const cfg = getAIConfig(env); if (cfg.provider === "openrouter" && !cfg.openrouter.apiKey) throw new AIError("AUTHENTICATION_FAILED", "OPENROUTER_API_KEY is required when AI_PROVIDER=openrouter"); return cfg; }
